...
var SHEET_SCHEMA = {
  '설정': ['키', '값'],
  '학생': ['학번', '이름', '짝학번', '입장시각'],
  '포스트잇': ['id', '학번', '유형', '내용', '매칭여부', '매칭상대학번', '숨김', '작성시각'],
  '매칭기록': ['id', '학생A학번', '학생B학번', '내용', '기록시각'],
  '성찰': ['학번', '줄때마음', '받을때마음', '작성시각']
};
...
function setupDatabase() {
  ...
  if (getSetting_('반코드') === null) setSetting_('반코드', String(Math.floor(1000 + Math.random() * 9000)));
  if (getSetting_('교사코드') === null) setSetting_('교사코드', String(Math.floor(100000 + Math.random() * 900000)));
  if (getSetting_('모드') === null) setSetting_('모드', '자유');
  if (getSetting_('상태') === null) setSetting_('상태', '대기');
  if (getSetting_('스포트라이트') === null) setSetting_('스포트라이트', '');
  touch_();
...
function findStudent_(sid) {
  ...
  for (var i = 0; i < rows.length; i++) {
    if (rows[i]['학번'] === String(sid)) return rows[i];
  }
  return null;
}
...
function buildManualSheet_() {
  ...
  var rows = [
    ...
    ['blank', '', ''],
    ['section', '4. 수업 진행 순서', ''],
    ['kv', '① 입장', '학생용 링크 + 반 코드(설정 시트 참고)를 칠판에 공유 → 학생들이 학번+이름으로 입장'],
    ...
    ['kv', '설정', '반코드/교사코드/모드/상태 등 (교사코드는 교사 화면 입장용 — 학생에게 알려주지 마세요)'],
    ['kv', '학생 / 포스트잇', '입장한 학생 / 작성된 포스트잇 (자동)'],
    ...
    ['kv', '반 코드를 바꾸고 싶어요', '메뉴 [④ 반 코드 재설정] 또는 설정 시트에서 직접 수정'],
    ['kv', '다음 반 수업에 또 쓰려면', '메뉴 [활동 데이터 전체 삭제] 후 상태를 [대기]로 — 학생·포스트잇·기록이 비워집니다'],
    ['kv', '테스트해보고 싶어요', '메뉴 [테스트 데이터 생성] → 가상 학생(김하늘·이도윤·박새봄)의 포스트잇이 생겨 전광판·교사 화면을 바로 볼 수 있어요'],
    ['blank', '', ''],
    ['section', '7. 저작권 및 문의', ''],
