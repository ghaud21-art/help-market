...
var SHEET_SCHEMA = {
  ...
  '성찰': ['session_id', '학번', '줄때마음', '받을때마음', '응원메시지', '작성시각']
};
...
function setupDatabase() {
  ...
  Object.keys(DEFAULT_SETTINGS).forEach(function (key) {
    if (getSetting_(key) === null) setSetting_(key, DEFAULT_SETTINGS[key]);
  });
  touch_();
...
function buildManualSheet_() {
  ...
  var rows = [
    ...
    ['text', '• 성찰 단계를 마치면 학생이 실제로 주고받은 도움 + 성찰 답변을 바탕으로 Gemini가 개인화된 응원 메시지를 만들어 주고, 학생은 이 내용이 담긴 카드를 이미지로 저장할 수 있습니다.', ''],
    ['blank', '', ''],
    ['section', '2. 사전 준비물', ''],
    ['kv', '구글 계정', '교사용 하나면 충분 (학생은 계정 불필요 — 반 코드+학번+이름 입장)'],
    ['kv', 'Gemini API 키', 'aistudio.google.com 에서 무료 발급 (성찰 응원 메시지 기능에 필요, 없어도 나머지 기능은 정상 동작)'],
    ['blank', '', ''],
    ['section', '3. 설치 방법 (★ 순서대로 5단계)', ''],
    ...
    ['kv', 'STEP 3', '[② Gemini API 키 설정] → 발급받은 키 붙여넣기 (건너뛰어도 되지만, 건너뛰면 성찰 응원 메시지는 나오지 않습니다)'],
    ['kv', 'STEP 4', '[확장 프로그램] → [Apps Script] → [배포] → [새 배포] → 유형 "웹 앱" → 실행: 나 / 액세스: 모든 사용자 → [배포] → URL 복사'],
    ['kv', 'STEP 5', '[🧺 도움 벼룩시장] → [④ 링크 만들기] → 복사한 URL 붙여넣기 → 학생용/전광판/교사용 링크가 자동 생성됩니다'],
    ['blank', '', ''],
    ['section', '4. 수업 진행 순서', ''],
    ...
    ['kv', '④ 성찰', '상태를 [성찰]로 → "도움을 줄 때/받을 때 어떤 마음이었나" 두 질문에 답 → 제출하면 AI 응원 메시지가 담긴 카드를 이미지로 저장'],
